...
from paradigm.runtime.utils import ensure_directory, iso_timestamp, make_json_safe, to_json_string
...
class CSVLogger:
    ...
    def write_row(self, row: dict[str, Any], flush: bool = True) -> None:
        serialized = {}
...
def setup_psychopy_logging(file_path: Path | None, level_name: str = "INFO") -> None:
    if file_path is None:
        return
    ensure_directory(file_path.parent)
    level = getattr(logging, level_name.upper(), logging.INFO)
    logging.console.setLevel(level)
    logging.LogFile(str(file_path), level=level, filemode="w")
