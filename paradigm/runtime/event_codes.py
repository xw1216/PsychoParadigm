...
def _build_task_events(items: list[tuple[str, int, str]]) -> dict[str, EventDefinition]:
    return {event_key: EventDefinition(event_key=event_key, event_code=event_code, description=description) for event_key, event_code, description in items}
...
EVENT_REGISTRY: dict[str, dict[str, EventDefinition]] = {
    ...
    "prl": _build_task_events(
        [
            ...
            ("prl.reversal.boundary", 39, "Contingency-switch boundary at the start of a new reversal block."),
    ...
    "rdm": _build_task_events(
        [
            ...
            ("rdm.feedback.incorrect", 57, "Trial-wise correctness feedback onset for an incorrect response."),
